...
class TextureHandler:
    def __init__(self):
        ...
        #-------------------------------------#
        debug_log.log(f"{assetsmarks.engine.debug}::textures.show_atlas_keys", 
                  value=list(self.atlas.data.keys())
                  )
        #-------------------------------------#
        signal_bus.subscribe(signals.TEXTURE_LOG_ATLAS_DATA, lambda: log_list(list(self.atlas.data.keys()), list_name="texture atlas data",console=True), priority=sig_prio.FIRST)
